...
def _window_partition(x: torch.Tensor, window_size: int) -> torch.Tensor: #입력 텐서 형식: (batch size, H: height, W: width, C: channel)
    ...
    #token: each data element in window
...
class WindowAttention(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        if dim % num_heads != 0: #in multihead attention, they divide the feature dims into num_heads so that each head takes a part of the features
            raise ValueError(
        ...
        #Make window: make a 8x8 grid (if window size=8) and flatten
        coords_h = torch.arange(window_size)
        coords_w = torch.arange(window_size)
        coords = torch.stack(torch.meshgrid(coords_h, coords_w, indexing="ij")) # shape (2, 8, 8)
        coords_flatten = coords.flatten(1) # flatten the 8×8 grid as a single-line token sequence; shape (2, 8x8)

        # For every token pair in window, compute relative coordinate (e.g. token i가 token j에 대해 어디에 있는지)
        # Shape (2, 64, 64)
        relative_coords = coords_flatten[:, :, None] - coords_flatten[:, None, :] #compute 
        relative_coords = relative_coords.permute(1, 2, 0).contiguous() # change shape to (64, 64, 2)

        #Relative Coords can be negative, so change range from [-7, 7] to [0, 14]
        relative_coords[:, :, 0] += window_size - 1 
        relative_coords[:, :, 1] += window_size - 1

        #token i와 token j 사이의 상대 위치가 table의 몇 번째 bias인지 나타내는 행렬
        #output shape: (64, 64)
        #bias: an extra learned number added to the raw attention score before softmax
            #bias helps learn not only how similar the token content is, but also considers position since in images, position matters a lot
        #this matrix stores which index to use in the bias table for each token pair (i, j) (e.g. use bias-table entry 37)
        relative_coords[:, :, 0] *= 2 * window_size - 1
        relative_position_index = relative_coords.sum(-1)

        #모델의 고정 텐서로 저장 - parameter는 아니지만 module state로 관리하고 싶은 값. 학습되는 파라미터는 아니지만 forward 때 계속 필요함.
        ...
        #qkv projection layer 만들기
        self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias) # 입력 feature로부터 query, key, value 를 만든다.
        self.attn_drop = nn.Dropout(attn_dropout) # attention이 특정 관계에 너무 과도하게 의존하지 않게 regularization을 한다.
        self.proj = nn.Linear(dim, dim) #multi-head attention 결과를 다시 하나의 feature representation으로 정리하기 위한 projection layer
        self.proj_drop = nn.Dropout(proj_dropout) # apply dropout once more

        nn.init.trunc_normal_(self.relative_position_bias_table, std=0.02) #relative_position_bias_table을 작은 랜덤값으로 초기화

    def forward( #performs the actual window-based multi-head self-attention
        self,
        x: torch.Tensor,
        mask: torch.Tensor | None = None,
    ) -> torch.Tensor:
        b_, n, c = x.shape
        #b_ = number of windows in the batch
        #n = number of tokens inside one window
        #c = feature dimension (dim)
        qkv = self.qkv(x).reshape(b_, n, 3, self.num_heads, self.head_dim) #produce q, k, v at once
        qkv = qkv.permute(2, 0, 3, 1, 4).contiguous() #rearrange dimensions so qkv[0] = query, qkv[1] = key, etc
        q, k, v = qkv[0], qkv[1], qkv[2]

        q = q * self.scale #scale the queries so that the dot products between q and k doesn't get too large
        attn = q @ k.transpose(-2, -1) #compute raw attention scores

        relative_bias = self.relative_position_bias_table[ #use the relative_position_index matrix to look up the learned relative-position bias for every token pair
            self.relative_position_index.view(-1)
        ]
        relative_bias = relative_bias.view( #reshape bias into (n, n, num_heads)
            self.window_size * self.window_size,
            self.window_size * self.window_size,
            self.num_heads,
        )
        relative_bias = relative_bias.permute(2, 0, 1).contiguous() #change shape to (num_heads, n, n) so it lines up with the attention tensor.
        attn = attn + relative_bias.unsqueeze(0) #add the relative-position bias to attention scores

        #optionally apply a mask for shifted window attention
        #some token pairs should not interact if they came from the same local region
        if mask is not None: 
        ...
        x = (attn @ v).transpose(1, 2).reshape(b_, n, c) #use attention weights to mix the values
        x = self.proj(x) #final linear projection
        x = self.proj_drop(x) #dropout again
        return x
